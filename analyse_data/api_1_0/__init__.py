...
success_header={'Content-type': 'application/json;charset=utf-8'}
...
def product_cmp_ab(a, b):
    ...
    return json.dumps(data), 200, success_header
...
def get_all_Apps():
    ...
    app_counter = 0
    for app in apps:
        apps_dict[app_counter]=app.get_dict()
        app_counter += 1
    return json.dumps(apps_dict), 200, success_header
...
#     "package": "com.youcash"
# }
@app.route('/appInformation/package=<package>')
def get_AppInformation_by_Package(package):
    app = DB_get_AppInformation_by_Package(package)
    if app == None:
        return json.dumps({})
    return json.dumps(app.get_dict()), 200, success_header


# 根据package，以json格式返回app在各应用市场的下载量
@app.route('/downloads_in_markets/package=<package>')
def get_AppDownloads_in_markets(package):
    ...
    return json.dumps(downloads_in_markets), 200, success_header
...
#     "2017-08-16": 90
# }
@app.route('/downloads/package=<package>/duration=<duration>')
def get_AppDownloads_on_days(package, duration):
    ...
    return json.dumps(downloads_on_days), 200, success_header
...
#     }
# }
@app.route('/versionHistory/package=<package>')
def get_VersionHistory(package):
    ...
    for version in versions:
        versions_dict[version_counter] = version.get_dict()
        version_counter += 1
    # 将日期转化为字符串供json处理
    for i in range(version_counter):
        versions_dict[i]['update_time'] = versions_dict[i]['update_time'].strftime("%Y-%m-%d %H:%M:%S")

    return json.dumps(versions_dict), 200, success_header
...
#     }
# }
@app.route('/scoreHistory/package=<package>/market=<market>/duration=<duration>')
def get_ScoreHistory(package, market, duration):
    ...
    return json.dumps(dict_of_comment_positive_negative), 200, success_header
